...
public class DirectoryViewController
{
    ...
    public void createDirectoryTree()
    {
        ...
        File buffer = codeTabPane.getCurrentFile();
        // only run this for bantam files
        if (buffer == null || !buffer.getName().endsWith("btm")) {
            return;
        }
        File file = this.fileMenuController.bufferManager.getOriginalFile(buffer);
