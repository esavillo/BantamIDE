...
public class Controller
{
    ...
    @FXML
    private Button                    assembleAndRunButton;
    @FXML
    ...
    @FXML
    private Button                    scanParseCheckButton;
    @FXML
    private Button                    cycleAnalysisButton;


    private ToolBarController.AssembleWorker assembleWorker;

    private ToolBarController.AssembleAndRunWorker assembleAndRunWorker;
    ...
    {
        ASTBroadcaster astBroadcaster = new ASTBroadcaster();
        ...
        this.structureViewController = new StructureViewController(astBroadcaster);
        ...
        this.codeTabPane.setASTBroadcaster(astBroadcaster);
        this.codeTabPane.setPostWriteFn(this::updateStructureView);

        // Sets focus to console on startup
        this.console.requestFocus();

        this.preferencesMenuController.loadTheme("/proj19DeutschDurstSavillo/resources/DarkTheme.css");
    }
    ...
    private void setupEventAwareness()
    {
        ...
        {
            this.codeTabPane.addEventFilter(KeyEvent.KEY_PRESSED, event ->
            {
        ...
        // Structure View various
        {
            ...
            this.checkBox.selectedProperty().addListener((observable, oldValue, newValue) ->
                                                         {
                                                             if (!newValue) { divider.setPosition(0.0); } else {
            ...
            this.codeTabPane.getSelectionModel().selectedItemProperty().addListener((observable, oldTab, newTab) ->
                                                                                    {
    ...
    {
        CustomCodeArea currentCodeArea = this.codeTabPane.getCurrentCodeArea();
    ...
    {
        ReadOnlyBooleanProperty isAssembling        = this.assembleWorker.runningProperty();
        ReadOnlyBooleanProperty isAssemblingRunning = this.assembleAndRunWorker.runningProperty();

        // disable stop button if not assembling and not running, or no files open
        this.stopButton.disableProperty().bind(
                ((isAssembling.not()).and(isAssemblingRunning.not()))
                        .or(this.fileMenuController.tablessProperty()));
        // disable assemble button if assembling or running, or no files open
        this.assembleButton.disableProperty().bind(
                isAssembling.or(isAssemblingRunning)
                        .or(this.fileMenuController.tablessProperty())
                        .or(this.fileMenuController.isMIPSProperty().not()));
        // disable assemble and run button if assembling or running, or no files open
        this.assembleAndRunButton.disableProperty().bind(
                isAssembling.or(isAssemblingRunning)
                        .or(this.fileMenuController.tablessProperty())
                        .or(this.fileMenuController.isMIPSProperty().not()));

        // disable scan and parse if tabless or not btm file
        this.scanButton.disableProperty().bind(
                Bindings.or(this.fileMenuController.tablessProperty(),
                            this.fileMenuController.isBTMProperty().not())
        );
        this.scanAndParseButton.disableProperty().bind(
                Bindings.or(this.fileMenuController.tablessProperty(),
                            this.fileMenuController.isBTMProperty().not())
        );
        this.scanParseCheckButton.disableProperty().bind(
                Bindings.or(this.fileMenuController.tablessProperty(),
                            this.fileMenuController.isBTMProperty().not())
        );
        this.cycleAnalysisButton.disableProperty().bind(
                Bindings.or(this.fileMenuController.tablessProperty(),
                        this.fileMenuController.isBTMProperty().not())
        );
    ...
    private void setupToolbarController()
    {
        ...
        this.assembleWorker = this.toolbarController.getAssembleWorker();
        this.assembleAndRunWorker = this.toolbarController.getAssembleAndRunWorker();
    }
    ...
    {
        this.fileMenuController.setTabPane(this.codeTabPane);
    ...
    {
        this.editMenuController.setTabPane(this.codeTabPane);
    }
    ...
    {
        // Initialize the dark theme menu item as being selected first, since this is the default theme
        this.preferencesMenuController = new PreferencesMenuController(this.darkThemeMenuItem);
    ...
    private void setupDirectoryController()
    {
        ...
        this.directoryViewController.setTabPane(this.codeTabPane);
    ...
    {
        this.toolbarController.handleScanButtonAction(event, this.codeTabPane.getCurrentTab());
    }
    ...
    {
        this.toolbarController.handleScanAndParseButtonAction(event, this.codeTabPane.getCurrentTab());
    }

    @FXML
    private void handleScanParseCheckButtonAction(Event event)
    {
        this.toolbarController.handleScanParseCheckButtonAction(event, this.codeTabPane.getCurrentTab());
    }

    @FXML
    private void handleCycleAnalysisButtonAction(Event event)
    {
        this.toolbarController.handleCycleAnalysisButtonAction(event, this.codeTabPane.getCurrentTab());
    }

    @FXML
    private void handleAssembleButtonAction(Event event)
    {
        this.toolbarController.handleAssembleButtonAction(event, this.codeTabPane.getCurrentTab().getFile());
    }

    @FXML
    private void handleAssembleAndRunButtonAction(Event event)
    {
        this.toolbarController.handleAssembleAndRunButtonAction(event, this.codeTabPane.getCurrentTab().getFile());
    }

    @FXML
    private void handleStopButtonAction(Event event)
    {
    ...
    {
        this.toolbarController.handleCompileButtonAction(event, this.codeTabPane.getCurrentTab());
    }
    ...
    {
        CustomCodeArea currentCodeArea = this.codeTabPane.getCurrentCodeArea();
        File           currentFile     = this.codeTabPane.getCurrentFile();
        ...
        if (currentCodeArea != null) {
            ...
            // if this is a java file
            {
                if (currentFile.getName().endsWith(".java") ||
                    ...
                    this.structureViewController.generateStructureTree(currentFile.getAbsolutePath());
                    return;
                }
            }
        }
    ...
    {
        this.structureViewController.handleTreeItemClicked(this.codeTabPane);
    }
