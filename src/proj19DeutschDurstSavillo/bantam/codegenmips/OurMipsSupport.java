/* Bantam Java Compiler and Language Toolset.
   ...
   Last Edit: May 2019 by Martin Deutsch,
                          Robert Durst, and
                          Evan Savillo.
*/
...
public class OurMipsSupport
{
     ...
     */
    public OurMipsSupport(PrintStream out)
    {
        this.out = out;
        this.program = new MipsProgram(out);
    }
    ...
    {
        this.program.dumpIntoOut();
    }

    public void setLastInstrComment(String comment)
    {
        this.program.setLastInstrComment(comment);
    }
    ...
    {
//        out.println("\t# " + text);
        program.writeComment(text);
    }

    /**
     * Generate a comment indented by the number specified as parameter indent
     *
     * @param text text to put in comment
     */
    public void genComment(String text, int indent)
    {
        StringBuilder builder = new StringBuilder();
        for (; indent > 0; --indent) {
            builder.append("    ");
        }
        program.writeComment(text.equals("") ? "" : builder.toString() + text);
    }
    ...
    {
//        out.println("\t.text");
        program.write(new Instruction(".text"));
        genGlobal("main");
        genGlobal("Main_init");
        genGlobal("Main.main");
...
//        out.println("\t.globl\t" + label);
        program.write(new Instruction(".globl", new String[] {label}));
    }
    ...
    {
//        out.println(label + ":");
        program.write(new Instruction(label + ":"));
    }
...
//         out.println("\tjal " + label);
        program.write(new Instruction("jal", new String[] {label}));
    }
    ...
    {
//        out.println("\t.data");
        program.write(new Instruction(".data"));
        genGlobal("gc_flag");
...
//        out.println("\t.word\t" + dataWord);
        program.write(new Instruction(".word", new String[] {dataWord}));
    }
    ...
    {
//        out.println("\t.space\t" + n);
        program.write(new Instruction(".space", new String[] {Integer.toString(n)}));
    }
...
//        }
         ...
         *      last modified:  May 2019, EAS */
        StringBuilder finalascii = new StringBuilder("\"");
        for (int i = 0; i < ascii.length(); ++i) {
            if (ascii.charAt(i) == '\\' & i < ascii.length() - 1) {
//                out.println("\"");
                finalascii.append("\"");
                program.write(new Instruction(".ascii", new String[] {finalascii.toString()}));
                finalascii = new StringBuilder("\"");
...
//                out.print(ascii.charAt(i));
                finalascii.append(ascii.charAt(i));
            }
        }
...
//        out.println("\t.byte\t0");
        genByte("0");
        genAlign();
//        out.println(); // pretty print newline
    }
...
//        out.println("\t.byte\t" + dataByte);
        program.write(new Instruction(".byte", new String[] {dataByte}));
    }
...
//        out.println("\t.align\t" + (getWordSize() / 2));
        String alignment = Integer.toString(getWordSize() / 2);
        program.write(new Instruction(".align", new String[] {alignment}));
    }
    ...
    public void genAsciiz(String ascii)
    {
        StringBuilder finalascii = new StringBuilder("\"");
        for (int i = 0; i < ascii.length(); i++) {
            if (ascii.charAt(i) == '\\' & i < ascii.length() - 1) {
        ...
        finalascii.append("\"");
        program.write(new Instruction(".asciiz", new String[] {finalascii.toString()}));
        genAlign();
//        out.println(); // pretty print newline
    }
    ...
    public void genLoadWord(String destReg, int offset, String baseReg)
    {
        ...
        program.write(new Instruction("lw", new String[] {destReg, offset + "(" + baseReg + ")"}));
    }
    ...
    public void genLoadByte(String destReg, int offset, String baseReg)
    {
        ...
        program.write(new Instruction("lb", new String[] {destReg, offset + "(" + baseReg + ")"}));
    }
    ...
    public void genStoreWord(String srcReg, int offset, String baseReg)
    {
        ...
        program.write(new Instruction("sw", new String[] {srcReg, offset + "(" + baseReg + ")"}));
    }
    ...
    public void genStoreByte(String srcReg, int offset, String baseReg)
    {
        ...
        program.write(new Instruction("sb", new String[] {srcReg, offset + "(" + baseReg + ")"}));
    }
    ...
    public void genLoadAddr(String destReg, String label)
    {
        ...
        program.write(new Instruction("la", new String[] {destReg, label}));
    }
     ...
     */
    public void genBinaryOp(String op, String destReg, String srcReg, String operand2)
    {
        checkReg(destReg);
        checkReg(srcReg);
        // out.println("\t" + op + " " + destReg + " " + srcReg + " " + operand2);
        program.write(new Instruction(op, new String[] {destReg, srcReg, operand2}));
    }
    ...
    private void genUnaryOp(String op, String destReg, String srcReg)
    {
        ...
        program.write(new Instruction(op, new String[] {destReg, srcReg}));
    }
...
//         out.println("\tjalr " + reg);
        program.write(new Instruction("jalr", new String[] {reg}));
    }
...
//         out.println("\tjr " + getRAReg());
        program.write(new Instruction("jr", new String[] {getRAReg()}));
    }

//    /**
//     * Generate a reference label with a comment
//     *
//     * @param label label string
//     */
//    public Instruction genLabel(String label, String comment) {
//        return new Instruction(label + ":", comment);
//    }
    ...
    {
//        out.println("\tb " + label);
        program.write(new Instruction("b", new String[] {label}));
    }
                           ...
                           String label)
    {
        ...
        program.write(new Instruction(op, new String[] {reg1, reg2, label}));
    }

    /**
     * Generate a conditional branch
     * branches if first operand is equal to zero
     *
     * @param reg1  first register to compare
     * @param label label to branch to
     */
    public void genCondBeqz(String reg1, String label)
    {
        genCondBr("beq", "$zero", reg1, label);
    }
    ...
    /**
     * Generate a conditional branch
     * branches if first operand is not equal to zero
     *
     * @param reg1  first register to compare
     * @param label label to branch to
     */
    public void genCondBnez(String reg1, String label)
    {
        genCondBr("bne", "$zero", reg1, label);
    }
...
//        out.println("\tsyscall");
        program.write(new Instruction("syscall"));
    }
    ...
    public void genLoadImm(String destReg, int imm)
    {
        ...
        program.write(new Instruction("li", new String[] {destReg, Integer.toString(imm)}));
    }
