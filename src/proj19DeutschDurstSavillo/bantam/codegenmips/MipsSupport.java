...
public class MipsSupport {
    ...
    public void genComment(String text, int indent) {
        StringBuilder builder = (indent == 0) ? new StringBuilder(" ") : new StringBuilder();
        for (; indent > 0; --indent) {
            builder.append("    ");
        }
        out.println(text == "" ? "" : "\t#" + builder.toString() + text);
    }

    /**
     * Generate a right fixed comment in order to create more aesthetic
     * documentation.
     *
     * @param text text to put in comment
     */
    public void genRightFixedComment(String text) {
        out.print("\033[40C"); // Move right 40 cols
        out.print("\033[1A"); // Move up 1 row
        out.println("\t# " + text);
    }
