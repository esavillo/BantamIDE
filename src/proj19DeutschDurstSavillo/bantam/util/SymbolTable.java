...
public class SymbolTable {
    ...
    public void cloneInto(SymbolTable symbolTable) {
        // clone parent symbol table (as well as all other ancestors)
        if (parent != null) {
            parent.cloneInto(symbolTable);
        }

        for (Hashtable<String, Object> table : scopes) {
            for (String key : table.keySet()) {
                symbolTable.add(key, table.get(key));
            }
    ...
    /**
     * Dump names of all variables from symbol table into a list
     * @return list of strings representing variable names
     */
    public List<String> dumpIntoList()
    {
        ArrayList<String> alist = new ArrayList<>();

        if (parent != null) {
            alist.addAll(parent.dumpIntoList());
        }

        scopes.forEach(hastable ->
                       {
                           alist.addAll(hastable.keySet());
                       });
     ...
     * @return list of strings representing variable names
     */
    public void dumpIntoList(List<String> alist)
    {
        if (parent != null) {
            alist.addAll(parent.dumpIntoList());
        }

        scopes.forEach(hashtable ->
                       {
                           alist.addAll(hashtable.keySet());
                       });
    }
}