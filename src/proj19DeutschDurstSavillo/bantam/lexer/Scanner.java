...
package proj19DeutschDurstSavillo.bantam.lexer;
...
public class Scanner
{
    ...
    private Token completeLongerToken()
    {
        ...
        boolean sloppy_workaround = false;
        ...
        switch (this.currentChar)
        {
            ...
            case '-': //token can be - or -- or -*>
                ...
                if (this.currentChar == '-') //check whether has second -
                {
                    ...
                    this.currentChar = this.sourceFile.getNextChar();
                    // Check if this is a right arrow
                    if (this.currentChar == '-' || this.currentChar == '>') {
                        sloppy_workaround = true;
                        kind = Token.Kind.RARROW;
                        spelling.append(this.currentChar);
                        ...
                        // RARROWs can be of arbitrary length.
                        while ((this.currentChar = this.sourceFile.getNextChar()) == '-') {
                            spelling.append(this.currentChar);
                        }

                        // There had better be a '>' at the end of this chain of -'s
                        if(this.currentChar == '>') {
                            spelling.append(this.currentChar);
                            this.currentChar = this.sourceFile.getNextChar();
                        } else {
            ...
            case '<': //token can be < or <= or <-*
                ...
                {
                    kind = Token.Kind.LARROW;
                    spelling.append(this.currentChar);

                    // LARROWs can be arbitrarily long, so eat up the -'s
                    while ((this.currentChar = this.sourceFile.getNextChar()) == '-') {
                        spelling.append(this.currentChar);
                    }
            ...
            case '!': // token can be compare or unarynot
                this.currentChar = this.sourceFile.getNextChar();
                if (this.currentChar == '=')
                {
                    spelling.append(this.currentChar);
    ...
    private Token completeBlockCommentToken(int lineNumber)
    {
        ...
        while (!terminated && this.currentChar != SourceFile.eof)
        {
            ...
            if (atTentativeEnd) // if '*' has been seen
            {
                ...
                else if (this.currentChar != '*') // otherwise just a '*' in the middle somewhere
                {
